...
parser.add_argument('--dataset', default='../../dataset_synapse/', type=str)  # cardiac
...
parser.add_argument('--load_path', default='checkpoints_synapse/UTbasex_n9e10l4+_06032227_0.8316172656078862.pth', type=str)
...
tf_train = JointTransform2D(img_size=args.imgsize, crop=crop, p_flip=0.0, p_rota=0.5, p_scale=0.0, p_gaussn=0.0,
...
trans_params = list(map(id, model.transformer.encoder.parameters()))
#trans_params = list(map(id, model.transformer.transformer.parameters()))
#trans_params = list(map(id, model.transformer.layers[:][0].parameters()))
base_params = filter(lambda p: id(p) not in trans_params, model.parameters())
params = [{'params': base_params},
          {'params': model.transformer.encoder.parameters(), 'lr': 1e-4}]
...
boardpath = './tensorboard_synapse/' + args.modelname + '_' + timestr
...
for epoch in range(args.epochs):
    ...
    for batch_idx, (X_batch, y_batch, *rest) in enumerate(dataloader):
        ...
        #y_batch_pool = rearrange(y_batch, 'b (h n) (w m) -> b h w (n m)', n=16, m=16)
        ...
        #train_loss = 0.9 * bcdice_loss + 0.1*head_loss
    ...
    if eval_mode == "slice":
        for batch_idx, (X_batch, y_batch, *rest) in enumerate(valloader):
            ...
            # val_head_loss = ortholoss(qkvs1)
            # val_attn_loss = rentropyloss(attns1)

            # val_losses += 0.9 * val_bcdice_loss.item() + 0.1 * val_attn_loss.item()
        ...
        print('epoch [{}/{}], test loss:{:.4f}'.format(epoch, args.epochs, val_losses / (batch_idx + 1)))
        TensorWriter.add_scalar('val_loss', val_losses / (batch_idx + 1), epoch)
        TensorWriter.add_scalar('dices', dices, epoch)
        if epoch == 70:
            for param in model.parameters():
                param.requires_grad = True
        if dices > best_dice or epoch == args.epochs - 1:
            best_dice = dices
            timestr = time.strftime('%m%d%H%M')
            save_path = './checkpoints_synapse/' + args.modelname + '_%s' % timestr + '_' + str(best_dice)
            torch.save(model.state_dict(), save_path + ".pth", _use_new_zipfile_serialization=False)